  ...
  return (
    <div className="max-w-[900px] mx-auto px-6 py-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-minbak-h2 font-semibold text-minbak-black">
          블로그 관리
        </h1>
        <Link
          href="/admin/blog/new"
          className="px-4 py-2 bg-minbak-primary text-white text-minbak-body font-medium rounded-minbak hover:bg-minbak-primary-hover transition-colors"
        >
          새 글 쓰기
        </Link>
      </div>
      ...
      {posts.length === 0 ? (
        <p className="text-minbak-body text-minbak-gray py-8">
          아직 글이 없습니다. 새 글을 작성해 보세요.
        </p>
      ) : (
        <ul className="border border-minbak-light-gray rounded-minbak overflow-hidden bg-white">
          {posts.map((post) => (
