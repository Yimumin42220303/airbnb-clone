...
import { sendScheduledMessageNow } from "@/lib/scheduled-messages";
...
export async function POST(
  ...
  if (conversation.booking.listing.userId !== userId) {
    return NextResponse.json({ error: "호스트만 전송할 수 있습니다." }, { status: 403 });
  }
  ...
  const result = await sendScheduledMessageNow(scheduleId, userId);
