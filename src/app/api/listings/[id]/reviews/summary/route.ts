...
1. **좋은 점** 3가지 (짧은 문장). 리뷰에 거의 없으면 1~2개만.
2. **아쉬운 점(단점)** 리뷰에서 실제로 언급된 것만 넣으세요. 없으면 빈 배열 []로 두세요. 억지로 개수를 채우지 마세요.
3. **이런 분에게 추천해요** 3~4가지: 리뷰 내용을 바탕으로 이 숙소가 특히 잘 맞는 게스트 유형을 짧게 나열.
반드시 JSON만 반환하세요. 다른 텍스트는 포함하지 마세요.
형식: {"pros": ["좋은 점1", "좋은 점2", ...], "cons": ["아쉬운 점1", ...] 또는 [], "recommendedFor": ["추천 대상1", ...]}`;

    const userPrompt = `## 이 숙소의 리뷰 (최근 ${reviews.length}개)\n\n${reviewTexts}\n\n위 리뷰를 바탕으로 좋은 점, 아쉬운 점(리뷰에 언급된 것만, 없으면 []), 이런 분에게 추천해요를 JSON으로 요약해 주세요.`;
