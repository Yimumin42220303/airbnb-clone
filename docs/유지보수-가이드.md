...
| 명령 | 설명 |
|------|------|
| `npm run deploy:cli` | **권장** 로컬→Vercel 직접 배포 (가장 확실) |
| `npm run deploy` | git push 후, Deploy Hook/API로 배포 트리거 (설정 시) |
| `npm run deploy:vercel` | Deploy Hook 또는 API로만 배포 트리거 (push 없이) |
| `npm run deploy:watch` | 파일 변경 감지 후 자동 push + 배포 트리거 |

**배포가 안 될 때**  
1. `.env`에 `VERCEL_ORG_ID`, `VERCEL_PROJECT_ID` 설정 후 `npm run deploy:cli`  
2. 팀 구분: tokyominbak.net = minbaktokyos, airbnb-clone-six-hazel-53 = chris-projects  
3. 자세한 내용: `docs/배포-가이드.md`, `docs/배포-Deploy-Hook-설정.md`
