...
| 명령 | 설명 |
|------|------|
| `npm run deploy` | git push 후, Deploy Hook/API로 Vercel 배포 트리거 (설정 시) |
| `npm run deploy:vercel` | Deploy Hook 또는 API로만 배포 트리거 (push 없이) |
| `npm run deploy -- --vercel` | Vercel CLI 직접 배포 (VERCEL_ORG_ID, VERCEL_PROJECT_ID 필요) |
| `npm run deploy:watch` | 파일 변경 감지 후 자동 push + 배포 트리거 |

**Deploy Hook 권장** (push 후 배포가 안 될 때):  
`docs/배포-Deploy-Hook-설정.md` 참고.  
Vercel → Settings → Git → Deploy Hooks에서 main 브랜치용 URL 생성 후 `.env`에 설정.

- push 후 Deploy Hook/API로 배포 트리거 → Git webhook 불안정해도 동작
...
| push 후 배포 안 됨 | `.env`에 VERCEL_DEPLOY_HOOK 설정 (Vercel → Git → Deploy Hooks). 없으면 VERCEL_TOKEN + `npm run deploy:vercel` |
