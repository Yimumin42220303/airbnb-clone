...
| 명령 | 설명 |
|------|------|
| `npm run deploy` | git add → commit → push (Vercel Git 연동으로 자동 배포) |
| `npm run deploy -- --vercel` | Vercel CLI 직접 배포 (VERCEL_ORG_ID, VERCEL_PROJECT_ID 필요) |
| `npm run deploy:watch` | 파일 변경 감지 후 자동 push (백그라운드 실행) |

**중요**: Vercel 프로젝트가 연결한 GitHub 저장소와 `git remote origin`이 **동일**해야 합니다. `deploy`/`deploy:watch` 실행 시 origin URL이 출력되니 확인하세요.

- push 후에도 배포가 안 되면: Vercel 대시보드 → 프로젝트 Settings → Git → 연결된 저장소 확인
- Git 연동이 다른 저장소를 보고 있으면: `npm run deploy -- --vercel` 사용 (`.env`에 VERCEL_ORG_ID, VERCEL_PROJECT_ID 설정 후)
...
| push 후 배포 안 됨 | `git remote -v` 로 origin URL 확인 → Vercel Settings → Git 연결 저장소와 일치 여부 확인. 불일치 시 `npm run deploy -- --vercel` 사용 |
